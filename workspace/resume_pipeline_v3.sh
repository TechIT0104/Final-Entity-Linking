#!/bin/bash
# ======================================================================
# MVD Pipeline — RESUME v3 from Stage 2 (Teacher)
# Fix: cand_num=8 for teacher (16*10=160 seqs OOM on 24GB; 8*10=80 fits)
#      Exclude GPU 2 (occupied by another user)
# ======================================================================
...
LOG=/home/kmpooja/entity_linking/mvd_resume_v3_${TIMESTAMP}.log
...
log "============================================================"
log "   MVD Pipeline RESUME v3 — Teacher cand_num=8 (24GB fix)"
log "============================================================"
...
# Detect free GPUs — exclude any with >1GB used
...
# Cross-encoder processes (batch * cand_num * max_view_num) sequences.
# Paper: cand_num=16 → 16*10=160 seqs → needs ~22GB per GPU.
# Our RTX 3090 has 24GB → OOM. Using cand_num=8 → 80 seqs → ~12GB.
# ======================================================================
log ""
log "============================="
log " STAGE 2: Train Teacher (cand_num=8)"
log "============================="
log "Started: $(date)"

cd $WORK

# Clean any leftover teacher output
...
# Bi-encoder is much lighter than cross-encoder, can use cand_num=16.
# But teacher forward pass inside MVD still runs cross-encoder on the
# candidates with views. For safety, use cand_num=8 here too.
# ======================================================================
log ""
log "============================="
log " STAGE 3: MVD Distillation (cand_num=8)"
log "============================="
log "Started: $(date)"
...
log ""
log "============================================================"
log "   MVD RESUME v3 Pipeline — COMPLETE"
log "   Finished: $(date)"
log "   Full log: $LOG"
log "============================================================"
