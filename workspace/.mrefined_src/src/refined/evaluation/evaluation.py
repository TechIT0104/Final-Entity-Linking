...
def process_annotated_document(
    ...
    for idx,span in enumerate(doc.spans):
        ...
        if validate_gold_offsets and span.gold_entity.wikipedia_entity_title:
            # Some TR2016 mentions contain corrupted offsets; keep only spans aligned with the gold mention text.
            gold_surface = str(span.gold_entity.wikipedia_entity_title).replace("_", " ")
            if normalize_surface_form(span.text) != normalize_surface_form(gold_surface):
                filtered_gold_spans += 1
                continue
    ...
    if validate_gold_offsets and filtered_gold_spans:
        LOG.debug(f"Filtered {filtered_gold_spans} gold spans with offset/text mismatch")
...
def evaluate_on_docs(
    ...
    for doc_idx, doc in tqdm(
        ...
        doc_metrics = process_annotated_document(
            ...
            topk_eval = topk_eval,
            top_k = top_k,
            validate_gold_offsets=validate_gold_offsets,
        )
