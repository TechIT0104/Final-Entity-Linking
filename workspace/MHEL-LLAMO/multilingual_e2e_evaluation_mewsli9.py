...
def main():
    ...
    parser.add_argument('--datasets_root', type=str,
                        ...
                        help="absolute or relative path to mewsli_9_el_datasets directory", required=False)
    parser.add_argument('--device', type=str,
                        default="cuda:0",
                        help="device to use (cuda:0, cpu, etc.)", required=False)
    ...
    print(f"Refined device: {refined.device}; cuda_available=True", flush=True)
    ...
    for lang in languages:
        ...
        datasets_dir = os.path.join(cli_args.datasets_root, lang)
        dataset_name = f'mewsli-9-{lang}'
        print(f"[LANG {lang}] start: datasets_dir={datasets_dir}", flush=True)
