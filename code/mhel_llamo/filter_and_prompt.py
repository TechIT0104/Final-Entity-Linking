...
def main():
    ...
    for item in tqdm(retriever_results):
        ...
        if args.threshold > 0 and item["candidates"][0]["score"] >= args.threshold:
            ...
            allowed_qids = {cand["wikidata_id"].upper() for cand in processed_candidates}
            ...
            wikidata_id = match.group(1).upper() if match else "NIL"
            if wikidata_id != "NIL" and wikidata_id not in allowed_qids:
                wikidata_id = "NIL"
